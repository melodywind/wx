...
class WxValidate {   
  ...
  __getFormatValidate(rule = {}, message = {},id, fieldname,validate){
    console.log("**** "+validate)
    const str = util.formatString('{0}[{1}]设置的验证有错误，请修改 ：{2}', new Array(fieldname,id,validate));      
    if (util.isEmpty(id) || util.isEmpty(fieldname) || util.isEmpty(validate)){
      util.alert('提示1',str)     
      return false;
    }
        ...
        switch (validateKey){          
          case "size":           
            ...
            if (reg.test(s)) {              
              ...
              message[validateKey] = util.formatString("【"+fieldname+"】请输入长度在 {0} 到 {1} 之间的字符！", rule[validateKey]);
            } else if (regMin.test(s)) {
              rule.minSize=value;
              message.minSize = util.formatString("【" + fieldname +"】最少要输入{0}个字符！", rule.minSize);
            } else if (regMax.test(s)) {
              rule.maxSize = value.replace(",","");
              message.maxSize = util.formatString("【" + fieldname +"】最多能输入{0}个字符！", rule.maxSize);
            }else{
              util.alert('提示3', str);
            }
            break;                              
          case "range":             
            ...
            if(reg.test(s)){             
              rule[validateKey] = value.split(",");
              message[validateKey] =  util.formatString("【" + fieldname +"】必须在{0}-{1}之间！", rule[validateKey]);
            }else{
              ...
              if(s.indexOf("[")>-1){
                ...
                if (!util.isEmpty(arrRangeValue[0])) {
                  ...
                  message.min =  util.formatString("【" + fieldname +"】请输入不小于 {0} 的数值！", rule.min);
                }
              } else if(s.indexOf("(") > -1) {
                if (!util.isEmpty(arrRangeValue[0])) {
                  rule.minLt = arrRangeValue[0];
                  message.minLt =  util.formatString("【" + fieldname +"】请输入大于 {0} 的数值！", rule.minLt);
                }
              }
              if (s.indexOf("]") > -1) {
                //[5]                
                if (arrRangeValue.length==2 && !util.isEmpty(arrRangeValue[1])) {
                  ...
                  message.max = util.formatString("【" + fieldname +"】请输入不大于 {0} 的数值！", rule.max);
                }
              } else if (s.indexOf(")") > -1) {
                if (arrRangeValue.length == 2 && !util.isEmpty(arrRangeValue[1])) {
                  rule.maxGt = arrRangeValue[1];
                  message.maxGt =  util.formatString("【" + fieldname +"】请输入小于 {0} 的数值！", rule.maxGt);
                }
              }
              }
            }         
          ...
          case "equals":
            ...
            message[validateKey] = "【" + fieldname +"】" + msg;
            break;
          case "notEmpty":            
          case "int":
          case "email":
          ...
          case "dateISO":        
            ...
            message[validateKey] = util.formatString("【{0}】" + this.defaults.messages[validateKey], fieldname) ;
            break;
          default:           
            util.alert('提示2', str);    
            break;
    ...
    __initDefaults() {
        this.defaults = {
            messages: {
                ...
                equals: this.formatTpl('输入值必须和 {0} 相同。')
            }
        }
    }
